...
func TestSACKRecovery(t *testing.T) {
	...
	// Turn off RACK to prevent TLPs from being sent.
	var rackOff tcpip.TCPRecovery
	if err := c.Stack().SetTransportProtocolOption(tcp.ProtocolNumber, &rackOff); err != nil {
		t.Fatalf("can not turn off RACK: %v", err)
	}
